...
def run_avoiding_obstacle(gyro, cap):
    ...
    # Inner-wall tripwire -- only relevant here, while avoidance steering could be
    # hugging the wall on the turn's own side.
    ...
    # is on black (tripped) this frame, green otherwise.
    if direction == "CWR":
    ...
    if all_contours:
        ...
        # Closeness: how far down the ROI the obstacle's bottom point sits -- 0 at the
        # top of the ROI (far away), 1 at the bottom (right in front of the robot).
        closeness = obstacle_rel_y / (middle_frame.y2 - middle_frame.y1)
        closeness = max(0.0, min(1.0, closeness))

        # Desired point: drawn as a line from the obstacle's own pass-side corner, growing
        # longer as the obstacle gets closer -- squaring closeness makes it grow quickly
        # rather than linearly. Capped at half the frame width. No wall lookup involved.
        max_line_len = cap.shape[1] // 2
        line_len = max_line_len * (closeness ** 2)
        target_x = int(obstacle_x - line_len) if obstacle_color == "GREEN" else int(obstacle_x + line_len)
        target_x = max(0, min(cap.shape[1] - 1, target_x))
        target_y = obstacle_y

        if SHOW_VID:
            cv2.line(cap, (obstacle_x, obstacle_y), (target_x, target_y), (255, 0, 255), 2)  # Pink
        ...
        if SHOW_VID:
            cv2.putText(cap, f"Error: {cam_error} px (closeness={closeness:.2f})", (target_x - 60, target_y - 15),
...
def run_turning(gyro, cap):
    ...
    view, turn() fires immediately and hands off to WALL_FOLLOW.
    ...
    the turn: turn() fires immediately (same frame it's first seen), and the
    obstacle-avoidance steering computed below
    ...
    if all_contours:
        ...
        if SHOW_VID:
            cv2.line(cap, (obstacle_x, obstacle_y), (target_x, target_y), (255, 0, 255), 2)
    ...
    # ---- turn execution: matching-colour veto, opposite-colour "reached the pass point"
    # gate. Skipped entirely the same frame REVERSING was just triggered above -- don't
    # also fire the turn while bailing out to back away. ----
    if state != State.REVERSING:
...
while True:
    ...
    if state == State.OUT_PARKING:
        ...
        if not park_square_filled:
            ...
            # TEMPORARY: just hold here once cleared instead of continuing into the
            # force turn -- everything below (force turn, final reverse, final turn)
            # is left in place, untouched, just unreachable while this flag is True.
            # Flip STOP_AFTER_PARK_CLEARED back to False to resume the rest of the sequence.
            steering = DEFAULT_STEER_ANGLE
            speed = 0
        elif not park_force_turn_done:
            # Forced max turn away from the pink marker just passed (130=CWR/right,
            # 50=CCL/left). Runs until the gyro shows a full 90 degrees of rotation
            ...
            # at steering=130. CCL now always mirrors CWR's own final turn here (same
    ...
    # short-circuits into the IN_PARKING branch at the top of the loop before it can
    # reach (and re-clobber) this point again.
    if orange_count >= LINE_COUNT or blue_count >= LINE_COUNT:
        if state != State.IN_PARKING:
            ...
            # no wall-follow steering, from this frame on.
            steering = DEFAULT_STEER_ANGLE
            speed = 0

...
park_saw_first_magenta = False  # true once a pink marker has been seen live -- park_cleared then
                                 # triggers the moment it drops out of view again (i.e. passed it)
...
def gyro_only_steer(gyro_heading, target_heading):
    ...
    the camera wall-following isn't reliable."""
    if gyro_heading is None:
        return DEFAULT_STEER_ANGLE
    return int(max(30, min(150, DEFAULT_STEER_ANGLE - KP_GYRO * angle_error(gyro_heading, target_heading))))
...
while True:
    ...
    if state == State.OUT_PARKING:
        ...
        if out_parking_start is None:
            ...
            out_parking_steer = 150 if left_area > right_area else 30
        ...
        if abs(sent_steer - steering) > 30 or speed != sent_speed or controller != sent_controller:
    ...
    if state == State.IN_PARKING:
        ...
        if not park_square_filled:
            ...
            # Start the forced turn as soon as we've passed the first pink marker --
            # i.e. it was visible, then dropped out of view -- rather than waiting for
            # both markers to be visible together.
            live_magenta = [c for c in magenta_contours if cv2.contourArea(c) > 600]
            if live_magenta:
                park_saw_first_magenta = True
            elif park_saw_first_magenta:
                park_cleared = True
            ...
            # Forced max turn away from the pink marker just passed (150=CWR/right,
            # 30=CCL/left). Runs until the gyro shows a full 90 degrees of rotation
            ...
            # Reverse straight for 1.3s at the end of the maneuver.
            if park_final_reverse_start is None:
                park_final_reverse_start = time.time()
            controller = "BWD"
            steering = DEFAULT_STEER_ANGLE
            speed = 85
            if time.time() - park_final_reverse_start >= 0.8:
                park_final_reverse_done = True
                controller = "FWD"
        elif not park_final_turn_done:
            # Forced max turn (150=CWR/right, 30=CCL/left), reversing while turning --
            ...
            # IN_PARKING is fully complete -- stop and end the program entirely.
            steering = DEFAULT_STEER_ANGLE
            speed = 0
            stop()
            ser.flush()
        ...
        if SHOW_VID:
            ...
            # Small debug square, 3/4 of the way up the frame (measured from the bottom).

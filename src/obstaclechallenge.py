# 1 ----------------------------------------------------------------------------------------
"""
I REMOVED THE THE 3 DEGREE THING -- CHECK CHECK CHECK
...
GYRO_STEER_DEADBAND = 5  # |heading_error| below this (degrees) -> gyro term holds straight (90) instead
                          # of continuing to correct; kills the steady-state P-controller wobble once
                          # heading is already close enough to desired_heading
...
park_reverse_start = None
park_reversed = False
park_straight_back_start = None
park_straight_back_done = False
park_final_reverse_start = None
park_final_reverse_done = False
...
def gyro_only_steer(gyro_heading, target_heading):
    """Gyro-only proportional steering, no camera term -- used during IN_PARKING where
    the camera wall-following isn't reliable. Wheels lock straight (90) once within
    GYRO_STEER_DEADBAND of target_heading instead of continuing to correct -- stops
    actively steering into the turn once the heading is basically reached, rather than
    fighting momentum back and forth across the setpoint."""
    if gyro_heading is None:
        return DEFAULT_STEER_ANGLE
    error = angle_error(gyro_heading, target_heading)
    if abs(error) < GYRO_STEER_DEADBAND:
        return DEFAULT_STEER_ANGLE
    return int(max(30, min(150, DEFAULT_STEER_ANGLE - KP_GYRO * error)))
...
def navigate_wall(gyro_heading, desired_heading=0, kp_scale=1.0):
    ...
    # and also holds straight once already within GYRO_STEER_DEADBAND -- otherwise the P-only
    # correction never fully zeroes out (steady-state wobble) and fights the camera term.
    if gyro_heading is not None:
        heading_error = angle_error(gyro_heading, desired_heading)
        if abs(heading_error) < GYRO_STEER_DEADBAND:
    ...
    if gyro_heading is not None:
        cv2.putText(cap, f"gyro={DEFAULT_STEER_ANGLE}-{KP_GYRO}*{heading_error:.1f}={gyro_steer:.0f}",
                    (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)
    else:
        cv2.putText(cap, "gyro=None (no heading reading)", (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)
    cv2.putText(cap, f"cam={DEFAULT_STEER_ANGLE}+{KP}*({left_area:.0f}-{right_area:.0f})={cam_steer:.0f}",
                (10, 148), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)
    cv2.putText(cap, f"steer={GYRO_WEIGHT}*{gyro_steer:.0f}+{CAM_WEIGHT}*{cam_steer:.0f}={steering_value:.0f}",
                (10, 166), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)
    cv2.putText(cap, f"heading: desired={desired_heading:.0f} current={gyro_heading_str}",
                (10, 184), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)
...
while True:
    ...
    if state == State.OUT_PARKING:
        ...
        if out_parking_start is None:
            ...
            left_bigger = left_area > right_area
            out_parking_steer = 150 if left_bigger else 30
    ...
    if state == State.IN_PARKING:
        ...
        cx = cap.shape[1] // 2
        half = PARK_SQUARE_SIZE // 2
        wall_x = None

        if not park_square_filled:
            black_mask = cv2.inRange(parking_frame.hsv, black_range[0][0], black_range[0][1])
            square_mask = black_mask[PARK_SQUARE_CENTER_Y - half:PARK_SQUARE_CENTER_Y + half, cx - half:cx + half]
            if cv2.countNonZero(square_mask) > 0.8 * square_mask.size:  # square mostly black -> filled
        ...
        if not park_square_filled:
            ...
            live_magenta = [c for c in magenta_contours if cv2.contourArea(c) > 200]
            if len(live_magenta) >= 2:
                park_saw_two_magenta = True
            if park_saw_two_magenta and len(live_magenta) == 0:
                park_cleared = True
                park_cleared_time = time.time()

            steering = gyro_only_steer(gyro, desired_heading)
            speed = 75
        elif time.time() - park_cleared_time < 1.0:
             steering = gyro_only_steer(gyro, desired_heading)
             speed = 70
        elif not park_reversed:
            if park_reverse_start is None:
                print(f"Forward beyond pink: {time.time() - park_cleared_time:.3f}s")
                park_reverse_start = time.time()
            controller = "BWD"
            steering = 30 if direction == "CWR" else 150
            speed = 75
            if time.time() - park_reverse_start >= 1.5:
                park_reversed = True
        elif not park_straight_back_done:
            if park_straight_back_start is None:
                park_straight_back_start = time.time()
            controller = "BWD"
            steering = 135 if direction == "CWR" else 45
            speed = 75
            if time.time() - park_straight_back_start >= 0.0:
                park_straight_back_done = True
                controller = "FWD"
        elif not park_final_reverse_done:
            if park_final_reverse_start is None:
                park_final_reverse_start = time.time()
            controller = "BWD"
            steering = 150 if direction == "CWR" else 30
            speed = 75
            gyro_squared = gyro is not None and abs(angle_error(gyro, desired_heading)) < 5
            if gyro_squared or time.time() - park_final_reverse_start >= 1.25:
        ...
        if SHOW_VID:
            ...
            # Small debug square, top 1/4 of the frame.
            cv2.rectangle(cap, (cx - half, PARK_SQUARE_CENTER_Y - half), (cx + half, PARK_SQUARE_CENTER_Y + half),
                          (255, 255, 255), 1)
            ...
            cv2.putText(cap, f"magenta_count={magenta_count} saw_two={park_saw_two_magenta} cleared={park_cleared}",
...
# ---- reversing logc ------
     ...
     ### obstacle avoidance -- finding the points to plot and follow
            ...
            if black_wall_x is not None:
                ...
                if state != State.REVERSING:
                    ...
                    steering = int(max(30, min(150, DEFAULT_STEER_ANGLE + kp * cam_error)))
    ...
    # Once either colour has been crossed LINE_COUNT times (the last line, already counted
